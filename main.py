...
#Local imports
from data.zsldataset import ZSL_dataset
...
def main():
    ...
    ### Init datasets
    trainset = ZSL_dataset(cfg, 'train')
    valset_unseen = ZSL_dataset(cfg, 'test_unseen')
    valset_seen = ZSL_dataset(cfg, 'test_seen')
    ...
    try:
        ...
        print('No cache found, Loading aux from text')
    ...
    if cfg.models.load != 'no' or args.evalmode:
        if args.evalmode and cfg.models.load == 'no':
            cfg.models.load = ospj(output_dir, 'ckpt_bestzsl.t7')
        print(f'Loading ckpt from {cfg.models.load}')
        ckpt = torch.load(cfg.models.load)
        model.load_state_dict(ckpt['net'])

    if cfg.train.ee_diff:
    ...
    if args.evalmode:
        print('Eval mode selected')
        model.eval()
        with torch.no_grad():
                evaluate(cfg, model, unseen_aux, unseen_aux_mask, unseen_mask, valloader_unseen, valloader_seen, writer, 0, results_zsl, results_gzsl, results_c_zsl, results_c_gzsl)
        return
...
def evaluate(cfg, model, unseen_aux, unseen_aux_mask, unseen_mask, valloader_unseen, valloader_seen, writer, epoch, results_zsl, results_gzsl, results_c_zsl, results_c_gzsl):
    ...
    def calibrated_stacking(preds_seen, preds_unseen):
        ...
        if cfg.models.unseen_calibration:
            calibration = [cfg.models.unseen_calibration]
        else:
            calibration = range(0,100)
        for a in tqdm(calibration, desc = 'Calibrated Stacking'):
            bias = a / 100.0
